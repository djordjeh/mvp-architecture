...
public class TaskLocalDataSource implements TaskDataSource {
    ...
    public Observable<List<Task>> tasks(boolean forceUpdate) {
        final String query = String.format("SELECT * FROM %s ORDER BY %s DESC", AppSQLiteHelper.TaskEntity.TABLE_NAME, AppSQLiteHelper.TaskEntity.ID);
    ...
    public Maybe<Task> task(long taskId) {
        final String query = String.format("SELECT * FROM %s WHERE %s = ?", AppSQLiteHelper.TaskEntity.TABLE_NAME, AppSQLiteHelper.TaskEntity.ID);
    ...
    public Single<Task> delete(@NonNull Task task) {
        final String whereClause = AppSQLiteHelper.TaskEntity.ID + " = ?";
    ...
    private ContentValues taskContent(@NonNull Task task) {
        ...
        values.put(AppSQLiteHelper.TaskEntity.ID, task.getId() == 0 ? System.currentTimeMillis() : task.getId());
        ...
        values.put(AppSQLiteHelper.TaskEntity.COMPLETED, task.getCompleted() ? 1 : 0);
        return values;
    }
